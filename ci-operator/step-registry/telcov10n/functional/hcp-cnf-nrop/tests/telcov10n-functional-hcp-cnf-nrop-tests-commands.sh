...
NROP_BRANCH="master"
GINKGO_LABEL="(!reboot_required && tier0)"
...
# TODO Install NROP Operator before applying the Performance Profile
echo "************ Installing NROP operator ************"
ansible-playbook -vv "${SHARED_DIR}"/repos/telco-ci/playbooks/hcp_nrop_install.yml -e kubeconfig="${SHARED_DIR}"/mgmt-kubeconfig -c local
...
echo "************ Running ${GINKGO_LABEL} NROP tests ************"
